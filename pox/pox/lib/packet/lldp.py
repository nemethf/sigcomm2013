...
class lldp (packet_base):
  ...
  def next_tlv(self, array, w = True, i = 0):
    ...
    if len(array) < length:
      if w:
        self.msg('(lldp tlv parse) warning TLV data too short to parse (%u)%s:'
                 % (len(array),i))
      return
  ...
  def parse (self, raw):
    ...
    # get PORT ID
    ret = self.next_tlv(raw[pduhead:], True, 2)
    if ret is None:
    ...
    # get  TTL
    ret = self.next_tlv(raw[pduhead:], True, 3)
    if ret == None:
    ...
    arr_len = len(raw)
    while True:
      ret = self.next_tlv(raw[pduhead:], False, 4)
      if ret == None:
